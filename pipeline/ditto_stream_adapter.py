...
class DittoStreamAdapter:
    ...
    def __init__(
        ...
        logger.info(f"[DittoStreamAdapter] Loading StreamSDK from {data_root} on {device} …")
        self.device = device
        self.sdk = StreamSDK(cfg_pkl, data_root, device=device)
    ...
    def _install_writer_patch(self):
        ...
        def _patched_writer_worker():
            ...
            import torch
            if hasattr(sdk, 'device_id') and torch.cuda.is_available():
                torch.cuda.set_device(sdk.device_id)
    ...
    def push_features(self, features: np.ndarray, seq: int = 0):
        ...
        _q_size_before = self.sdk.audio2motion_queue.qsize()
        _t_push = time.perf_counter()
        
        # timeout=1.0: allows Ditto to burst-process without immediately shredding chunks
        try:
            self.sdk.audio2motion_queue.put(features, timeout=1.0)
        except queue.Full:
            logger.error(
    ...
    def iter_frames(self) -> Iterator[bytes]:
        ...
        while True:
            ...
            logger.debug(
                ...
                f"jpeg_kb={len(item)//1024} | fps={_fps_win:.1f} | "
                f"frame_q={self._frame_queue.qsize()}"
            )
