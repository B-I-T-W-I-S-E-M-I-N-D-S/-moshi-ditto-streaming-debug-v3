...
class Config:
    ...
    DITTO_DEVICE: str = "cuda:1" if torch.cuda.is_available() and torch.cuda.device_count() > 1 else DEVICE
...
def _run_bridge_step(bridge: StreamingBridgeInference, chunk: torch.Tensor) -> np.ndarray:
    ...
    if cfg.DEVICE.startswith("cuda") and _bridge_cuda_stream is not None:
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    # Strictly bound token queue to prevent 60-frame backlogs causing oscillations
    token_queue: asyncio.Queue = asyncio.Queue(maxsize=10)
    ...
    async def frame_forwarder_task():
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.warning(
                        f"[frame_forwarder] send_queue full — frame dropped | total_drops={_drop_count}"
                    )
                ...
                if frame_count % 50 == 0:
                    profiler.print_summary(
                        label=f"@ frame={frame_count} fps={_cur_fps:.1f}"
                    )
        ...
        logger.info(
            ...
            f"Forwarded={frame_count} dropped={_drop_count} final_fps={_cur_fps:.1f}"
        )
...
async def load_models():
    ...
    if cfg.DEVICE.startswith("cuda"):
        import torch
        # Disable cuDNN benchmark due to CUDNN_STATUS_INTERNAL_ERROR on A100
        # when running concurrent priority streams with TensorRT.
        torch.backends.cudnn.benchmark = False
        # Avoid blocking the event loop with CUDA synchronisation.
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.backends.cudnn.allow_tf32 = True
        logger.info("  cuDNN benchmark + TF32 enabled (A100 optimised)")
    # Pin CPU threads to the number of physical cores (avoids hyper-thread
    # contention on the 32-vCPU AMD EPYC pod).
    import torch
    ...
    if cfg.DEVICE.startswith("cuda"):
        # We ensure the stream is created on the correct Moshi/Bridge GPU
        import torch
        # By default torch.cuda.Stream() creates it on current device. We'll set device context temporarily.
    ...
    try:
        if hasattr(torch, 'compile'):
            ...
            logger.info("[2/2]   torch.compile applied to Bridge (reduce-overhead mode)")
